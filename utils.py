...
def get_wenet_encoder(device=None):
   print("Getting wenet encoding")
   from vencoder.WeNet import WeNet
   wenet = WeNet(device=device)
   return wenet
...
def ReTrans(source_feats, padded_target_len, model, sample_rate, hop_size):
        ...
        if model == 'wenet':
           source_hop = wenet_frameshift * wenet_downsample_rate * sample_rate

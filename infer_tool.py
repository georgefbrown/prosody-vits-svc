...
import sys
from modules.mel_processing import spectrogram_torch, mel_spectrogram_torch, spec_to_mel_torch
...
class Svc(object):
    def __init__(self, net_g_path, config_path,
        ...
        if not self.only_diffusion:
            ...
            #wenet model
            self.wenet_model = utils.get_wenet_encoder()
    ...
    def get_unit_f0(self, wav, tran, cluster_infer_ratio, speaker, f0_filter ,f0_predictor,cr_threshold=0.05, sr=44100):
        ...
        we = self.wenet_model.encoder(wav16k)
        we = utils.ReTrans(we, f0.shape[1], 'wenet', 44100, 512)
        we = we.transpose(1,2).squeeze(0)

        raw = torch.from_numpy(raw).unsqueeze(0)
        spec = spectrogram_torch(raw, hps.data.filter_length, 44100, 512, hps.data.win_length, center=False).to(self.dev)
        spec = torch.squeeze(spec, 0).to(self.dev)
    ...
    def infer(self, speaker, tran, raw_path,
        ...
        if spk_mix:
            c, w, we, f0, uv, p = self.get_unit_f0(wav, tran, 0, None, f0_filter,f0_predictor,cr_threshold=cr_threshold, sr=sr)
            n_frames = f0.size(1)
            ...
            c, w, we, mel, f0, uv, p = self.get_unit_f0(wav, tran, cluster_infer_ratio, speaker, f0_filter,f0_predictor,cr_threshold=cr_threshold)
            n_frames = f0.size(1)
        c = c.to(self.dtype)
        w = w.to(self.dtype)
        we = we.to(self.dtype)
        ...
        with torch.no_grad():
            ...
            if not self.only_diffusion:
                ...
                audio,f0 = self.net_g_ms.infer(c, w, we, f0=f0, g=sid, uv=uv, p=p, mel=mel, predict_f0=auto_predict_f0, noice_scale=noice_scale,vol=vol)
